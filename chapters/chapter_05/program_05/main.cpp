    ...
    auto worker = [ &init_thread ](int i) {
        ...
        if (init_value == 0) {
            ...
            if ( init_thread.compare_exchange_strong( expected, i, std::memory_order::seq_cst ) ) {
                ...
                sync_cout << "Previous value of init_thread: " << expected << "\n";
                sync_cout << "Thread " << i << " initialized\n" << std::flush;

            } else {

                sync_cout << "Thread " << i << " already initialized 2\n" << std::flush;
            }
        } else {
            // init_thread was already initialized
            sync_cout << "Thread " << i << " already initialized 1\n" << std::flush;
        }
    };
